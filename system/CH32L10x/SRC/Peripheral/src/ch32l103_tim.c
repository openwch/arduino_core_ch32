...
void TIM_TimeBaseInit(TIM_TypeDef *TIMx, TIM_TimeBaseInitTypeDef *TIM_TimeBaseInitStruct)
{
    ...
    if(TIMx == TIM4) TIMx->ATRLR_TIM4 = TIM_TimeBaseInitStruct->TIM_Period;
    else TIMx->ATRLR = (uint16_t)TIM_TimeBaseInitStruct->TIM_Period;
...
void TIM_OC1Init(TIM_TypeDef *TIMx, TIM_OCInitTypeDef *TIM_OCInitStruct)
{
    ...
    if(TIMx == TIM4) TIMx->CH1CVR_TIM4 = TIM_OCInitStruct->TIM_Pulse;
    else TIMx->CH1CVR = (uint16_t)TIM_OCInitStruct->TIM_Pulse;
    TIMx->CCER = tmpccer;
}
...
void TIM_OC2Init(TIM_TypeDef *TIMx, TIM_OCInitTypeDef *TIM_OCInitStruct)
{
    ...
    TIMx->CTLR2 = tmpcr2;
    TIMx->CHCTLR1 = tmpccmrx;
    if(TIMx == TIM4) TIMx->CH2CVR_TIM4 = TIM_OCInitStruct->TIM_Pulse;
    else TIMx->CH2CVR = (uint16_t)TIM_OCInitStruct->TIM_Pulse;
    TIMx->CCER = tmpccer;
}
...
void TIM_OC3Init(TIM_TypeDef *TIMx, TIM_OCInitTypeDef *TIM_OCInitStruct)
{
    ...
    if(TIMx == TIM4) TIMx->CH3CVR_TIM4 = TIM_OCInitStruct->TIM_Pulse;
    else TIMx->CH3CVR = (uint16_t)TIM_OCInitStruct->TIM_Pulse;
    TIMx->CCER = tmpccer;
}
...
void TIM_OC4Init(TIM_TypeDef *TIMx, TIM_OCInitTypeDef *TIM_OCInitStruct)
{
    ...
    TIMx->CTLR2 = tmpcr2;
    TIMx->CHCTLR2 = tmpccmrx;
    if(TIMx == TIM4) TIMx->CH4CVR_TIM4 = TIM_OCInitStruct->TIM_Pulse;
    else TIMx->CH4CVR = (uint16_t)TIM_OCInitStruct->TIM_Pulse;
    TIMx->CCER = tmpccer;
}
...
{
    if(TIMx == TIM4) TIMx->CNT_TIM4 = Counter;
    else TIMx->CNT = (uint16_t)Counter;
}
...
{
    if(TIMx == TIM4) TIMx->ATRLR_TIM4 = Autoreload;
    else TIMx->ATRLR = (uint16_t)Autoreload;
}
...
{
    if(TIMx == TIM4) TIMx->CH1CVR_TIM4 = Compare1;
    else TIMx->CH1CVR = (uint16_t)Compare1;
}
...
{
    if(TIMx == TIM4) TIMx->CH2CVR_TIM4 = Compare2;
    else TIMx->CH2CVR = (uint16_t)Compare2;
}
...
{
    if(TIMx == TIM4) TIMx->CH3CVR_TIM4 = Compare3;
    else TIMx->CH3CVR = (uint16_t)Compare3;
}
...
{
    if(TIMx == TIM4) TIMx->CH4CVR_TIM4 = Compare4;
    else TIMx->CH4CVR = (uint16_t)Compare4;
}
...
{
    if(TIMx == TIM4) return TIMx->CH1CVR_TIM4;
    else return TIMx->CH1CVR;
}
...
{
    if(TIMx == TIM4) return TIMx->CH2CVR_TIM4;
    else return TIMx->CH2CVR;
}
...
{
    if(TIMx == TIM4) return TIMx->CH3CVR_TIM4;
    else return TIMx->CH3CVR;
}
...
{
    if(TIMx == TIM4) return TIMx->CH4CVR_TIM4;
    else return TIMx->CH4CVR;
}
...
{
    if(TIMx == TIM4) return TIMx->CNT_TIM4;
    else return TIMx->CNT;
}
...
/*********************************************************************
 * @fn      TIM_CaptureLevelIndicate_Cmd
 *
 * @brief   Enables or disables the TIMx Capture Level Indicate function.
 *
 * @param   TIMx - where x can be 1 to 4 select the TIM peripheral.
 *          NewState - ENABLE or DISABLE.
 *
...
/*********************************************************************
 * @fn      TIM_CaptureModeConfig
 *
 * @brief   Configures the TIMx capture mode.
 *
 * @param   TIMx - where x can be 1 to 4 select the TIM peripheral.
 *          TIM_CaptureMode - the TIMx capture mode.
 *            TIM_Capture_Mode0 - When the counter overflows, capture count
 *        as actual count value.
 *            TIM_Capture_Mode1 - When the counter overflows, capture count
 *        as 0xFFFF.
 *
 * @return  none
 */
...
{
    TIMx->CTLR1 &= ~(TIM_Capture_Mode1);

    if(TIM_CaptureMode == TIM_Capture_Mode1)
    {
        TIMx->CTLR1 |= TIM_Capture_Mode1;
    }
}
