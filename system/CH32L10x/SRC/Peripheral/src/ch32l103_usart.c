...
void USART_DeInit(USART_TypeDef *USARTx)
{
    ...
    {
        RCC_PB1PeriphResetCmd(RCC_PB1Periph_USART4, ENABLE);
        RCC_PB1PeriphResetCmd(RCC_PB1Periph_USART4, DISABLE);
    }
}
...
{
    uint32_t          tmpreg = 0x00, PBclock = 0x00;
    ...
    {
        PBclock = RCC_ClocksStatus.PCLK2_Frequency;
    }
    else
    {
        PBclock = RCC_ClocksStatus.PCLK1_Frequency;
    }

    if((USARTx->CTLR1 & CTLR1_OVER8_Set) != 0)
    {
        integerdivider = ((25 * PBclock) / (2 * (USART_InitStruct->USART_BaudRate)));
    }
    else
    {
        integerdivider = ((25 * PBclock) / (4 * (USART_InitStruct->USART_BaudRate)));
    }
 ...
 *            USART_FLAG_RX_BUSY - Receive data busy flag.
 *            USART_FLAG_CTS - CTS state detection flag.
 ...
 *            USART_IT_CTS - CTS state detection interrupt.
 *            USART_IT_LBD - LIN Break detection interrupt.
